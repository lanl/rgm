#
# 3D isosurface rendering (pyvista) of the karst cave-network mask of the
# 3D karst example (example_3d_karst_1_151x201x251.bin), colored by depth,
# with a labeled axis box (depth increasing downward).
#
import numpy as np
...
vol = np.fromfile('../tmp/example_3d_karst_1_151x201x251.bin',
                  dtype=np.float32).reshape((n1, n2, n3), order='F')

# axes: x = x3, y = x2, z = depth (downward-positive; camera view-up flipped)
...
p.reset_camera()
p.camera.zoom(1.12)
p.screenshot('../fig_karst_iso.png')
print('saved fig_karst_iso.png')
